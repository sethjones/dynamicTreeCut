...
def cutreeHybrid(link, distM,
    ...
    if type(deepSplit) == bool:
        deepSplit = int(deepSplit) * (nSplitDefaults - 2)
    ...
    for clust in range(nBranches):
        ...
        if branch_failSize[clust]:
            SmallLabels[branch_singletons[clust][branch_singletons[clust] != 0] - 1] = clust + 1

    if not respectSmallClusters:
        SmallLabels = np.repeat(0, nPoints)

    if verbose > 2:
        print(spaces, "..Assigning Tree Cut stage labels..")
    ...
    if pamStage and UnlabeledExist and nProperLabels > 0:
        if verbose > 2:
            print(spaces, "..Assigning PAM stage labels..")
        nPAMed = 0
        ...
        if verbose > 2:
            print("....assigned", nPAMed, "objects to existing clusters.")
